 ...
 * `os = win | osx | linux`
 */
const copyTablesFromServer = async () => {
  ...
  const serverPath =
    channel === "insider"
    ? "/optolith-tables-insider"
    : "/optolith-tables"

  const fileLocaleRx = /TDE5_([a-z]{2}-[A-Z]{2})\.xlsx/

  const filesAndDirs = await Client.list (serverPath)
  const files =
    filesAndDirs .filter (fileInfo => fileInfo.isFile)
                 .map (fileInfo => {
                   const isUniv = fileInfo.name === "TDE5.xlsx"

                   const pathInTravis =
                     isUniv
                     ? path.join ("..", "app", "Database", "univ.xlsx")
                     : path.join ("..", "app", "Database", fileLocaleRx .exec (fileInfo.name) [0], "l10n.xlsx")

                   return Client.download (
                     fs.createWriteStream (pathInTravis),
                     `${serverPath}/${fileInfo.name}`
                   )
                 })
...
module.exports = copyTablesFromServer

copyTablesFromServer ()
