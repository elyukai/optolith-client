...
import { ActivatableInstance, ActivateArgs, DeactiveViewObject, DisadvantageInstance, InputTextEvent, Instance, SelectionObject, SkillishInstance, TalentInstance, UIMessages } from '../types/data';
    ...
    switch (id) {
      ...
      case 'SA_699':
        ...
        if (typeof selected === 'number') {
          ...
          const { spec, specInput, cost: selCost } = selectedObject;
          if (typeof specInput === 'string') {
            input = specInput;
            args.sel2 = inputText;
          }
          else {
            sel2 = sortObjects(spec!.map((name, i) => {
              return { id: i + 1, name };
            }), locale.id);
            args.sel2 = selected2;
          }
          currentCost = typeof selCost === 'number' ? selCost : cost as number;
        }
        break;
      default:
