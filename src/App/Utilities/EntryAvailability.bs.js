...
function insertPages(param) {
  ...
  var partial_arg = Ley_Ix$OptolithClient.range(/* tuple */[
        fst,
        lst
      ]);
...
function groupRanges(pages) {
  return Ley_List$OptolithClient.reverse(Ley_List$OptolithClient.Foldable.foldr((function (page, groupedReversedPages) {
                    if (!groupedReversedPages) {
                      return /* :: */[
                              /* Single */Block.__(0, [page]),
                              /* [] */0
                            ];
                    }
                    var prevPage = groupedReversedPages[0];
                    if (prevPage.tag) {
                      var prevLastPage = prevPage[1];
...
function showPages(pages) {
  return Ley_List$OptolithClient.intercalate(", ", Ley_List$OptolithClient.map((function (page) {
                    if (page.tag) {
                      return Ley_Int$OptolithClient.show(page[0]) + (Chars$OptolithClient.ndash + Ley_Int$OptolithClient.show(page[1]));
                    } else {
                      return Ley_Int$OptolithClient.show(page[0]);
                    }
                  }), pages));
}
