...
export function getPropertiesXML() : string {
  return "<?xml version=\"1.0\"?><map><entry><string>version</string><string>1.7.0</string></entry><entry><string>herolab</string><boolean>false</boolean></entry></map>"
}

export function getContentXML(hero: Readonly<Required<HeroModel>>) : string {
  var contentXml:string = "<?xml version=\"1.0\"?>"
...
function getAttributesXML(hero: Readonly<Required<HeroModel>>) : string {
  ...
  const leP:number = 30, asp:number = 0, kap:number = 0
  const schips:number = 3
  const sk:number = 1, zk:number = 1
  const ini:number = 12
  const gs:number = 8
...
function getCombatXML(hero: Readonly<Required<HeroModel>>) : string {
  ...
  var text:string = "WaffenLaufIndex=1;"
  //Für jede Waffe
...
function getBelongingsXML(hero: Readonly<Required<HeroModel>>) : string {
  ...
  const imisc:string = "{\"behaelter1\":\"Rucksack\",\"behaelter2\":\"Guerteltasche\",\"behaelter3\":\"Beutel\",\"behaelter4\":\"Am Koerper getragen\",\"behaelter5\":\"Satteltaschen\",\"dukaten\":"+purse.d+",\"silbertaler\":"+purse.s+",\"heller\":"+purse.h+",\"kreuzer\":"+purse.k+"}"
...
function entry(entry:MaptoolsEntry) : string {
  var xml:string = ""
  xml += "<entry>"
  xml += "<string>" + entry.key.toLowerCase() + "</string>"
  xml += "<net.rptools.CaseInsensitiveHashMap_-KeyValue>"
  xml += "<key>" + entry.key + "</key>"
  xml += "<value class=\"string\">" + entry.value + "</value>"
