...
export const {
  actions: {
    addInstance: addAncestorGlyph,
    changeInstanceLevel: changeAncestorGlyphLevel,
    removeInstance: removeAncestorGlyph,
  },
  reducer: ancestorGlyphsReducer,
} = createActivatableSlice({
  entityName: "AncestorGlyph",
  getState: state => state.specialAbilities.ancestorGlyphs,
  getPrerequisites: (id, database) => database.ancestorGlyphs[id]?.prerequisites ?? [],
  getAdventurePointsValue: staticEntry => staticEntry.ap_value,
  createIdentifierObject: getCreateIdentifierObject("AncestorGlyph"),
