...
import { _translate, translate } from '../utils/I18n';
...
export const getSpecialAbility = (state: Map<string, Data.SpecialAbilityInstance>, id: string) => state.get(id);
export const getSpecialAbilities = (state: AppState) => state.currentHero.present.dependent.specialAbilities;
export const getAdv = (state: Map<string, Data.AdvantageInstance>, id: string) => state.get(id);
export const getAdvantagesState = (state: AppState) => state.currentHero.present.dependent.advantages;
export const getDisadv = (state: Map<string, Data.DisadvantageInstance>, id: string) => state.get(id);
export const getDisadvantagesState = (state: AppState) => state.currentHero.present.dependent.disadvantages;
...
export function getActiveForView(state: CurrentHeroInstanceState, category: Categories.ACTIVATABLE): Data.ActiveViewObject[] {
	const { dependent, el } = state;
	const allEntries = getMapByCategory(dependent, category) as Map<string, Data.ActivatableInstance & { gr?: number; tiers?: number; }>;
...
export function getDeactiveForView(state: CurrentHeroInstanceState, category: Categories.ACTIVATABLE): Data.DeactiveViewObject[] {
	...
	allEntries.forEach(entry => {
		...
		if (isActivatable(state, a) && !dependencies.includes(false) && (max === undefined || active.length < max)) {
			switch (id) {
				...
				case 'SA_97': {
					const activeIds = getSids(a);
					const sel = sort(a.sel!.filter(e => getSids(get(dependent, 'SA_88') as Data.SpecialAbilityInstance).includes(e.id) && !activeIds.includes(e.id)), 'name');
					if (sel.length > 0) {
						finalEntries.push({ id, name, sel, cost, gr, instance: entry });
					}
					break;
				...
				case 'SA_103': {
					...
					const sel = sort(a.sel!.filter(e => getTraditionOfAspect(e.id as number) === activeTradition && counter.get(e.id as number)! >= 3 && !activeIds.includes(e.id) && !getDSids(a).includes(e.id)));
					if (sel.length > 0) {
...
export const getAdvantagesRating = createSelector(
	...
	(race, culture, profession) => {
		...
		if (race && culture && profession) {
			...
			race.importantAdvantages.forEach(e => { rating[e] = 'IMP'; });
		}
...
export const getDisadvantagesRating = createSelector(
	...
	(race, culture, profession) => {
		...
		if (race && culture && profession) {
			...
			race.importantDisadvantages.forEach(e => { rating[e] = 'IMP'; });
		}
...
export const getAdvantagesForSheet = createSelector(
	[ getPresent ],
	currentHero => {
		return getActiveForView(currentHero, Categories.ADVANTAGES);
	}
);
...
export const getDisadvantagesForSheet = createSelector(
	[ getPresent ],
	currentHero => {
		return getActiveForView(currentHero, Categories.DISADVANTAGES);
	}
);
...
export const getSpecialAbilitiesForSheet = createSelector(
	[ getPresent ],
	currentHero => {
		return getActiveForView(currentHero, Categories.SPECIAL_ABILITIES);
	}
);
