...
import { mergeIntoState } from '../utils/ListUtils';
	...
	switch (action.type) {
		...
		case ActionTypes.LOAD_HERO: {
			...
			for (const [id, value] of Object.entries(spells)) {
				newstate = mergeIntoState(newstate, addDependencies(newstate, { ...newstate.spells.get(id)!, active: true, value }));
			}
			...
			for (const id of blessings) {
				newstate = mergeIntoState(newstate, addDependencies(newstate, { ...newstate.blessings.get(id)!, active: true }));
			}

			for (const id of cantrips) {
				newstate = mergeIntoState(newstate, addDependencies(newstate, { ...newstate.cantrips.get(id)!, active: true }));
			}
			...
			for (const [id, active] of Object.entries(activatable)) {
				...
				switch (id) {
					...
					case 'DISADV_48':
						active.forEach(p => {
							newstate = mergeIntoState(newstate, addDependencies(newstate, entry, [], p.sid as string));
						});
						break;
					case 'SA_10': {
						...
						active.forEach(p => {
							if (typeof p.sid === 'string') {
								...
								if (amount) {
									...
									newstate = mergeIntoState(newstate, addDependencies(newstate, entry, [addRequire]));
								}
							}
						});
						break;
					...
					default:
						active.forEach(() => {
							newstate = mergeIntoState(newstate, addDependencies(newstate, entry));
						});
				}
			}
