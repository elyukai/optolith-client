...
import { ActiveObject, ToListById } from '../types/data.d';
...
export function convertHero(hero: RawHero): RawHero {
  const entry = { ...hero };
  if (lt(entry.clientVersion, '0.49.5')) {
    const newActivatable: ToListById<ActiveObject[]> = {};

    const convertIds: ToListById<ActiveObject & { id: string }> = {
      'SA_6': {id: 'SA_5', tier: 2},
      'SA_7': {id: 'SA_6'},
      'SA_8': {id: 'SA_7'},
      'SA_9': {id: 'SA_8'},
