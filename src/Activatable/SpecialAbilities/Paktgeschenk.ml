...
module Static = struct
  ...
  module Decode = Shared.Decode.Make (struct
    ...
    let permanentDemonicConsumption =
      Json.Decode.(
        field "type" string
        |> andThen (function
             | "Fixed" ->
                 field "value" int
                 |> map (fun x -> (Fixed x : permanentDemonicConsumption))
             | "PerLevel" -> field "value" int |> map (fun x -> PerLevel x)
             | str ->
                 raise
                   (DecodeError
                      ("Unknown permanent demonic consumption type: " ^ str))))

    let automaticEntryTarget =
      Json.Decode.(
        field "type" string
        |> andThen (function
             | "Selection" ->
                 ...
                 |> map (function
                      ...
                      | str ->
                          raise
                            (DecodeError
                               ("Unknown target selection type: " ^ str)))
             | "Fixed" ->
                 field "value" Id.Activatable.Decode.t
                 |> map (fun x -> (Fixed x : automaticEntryTarget))
             | str ->
                 raise
                   (DecodeError
                      ("Unknown permanent demonic consumption type: " ^ str))))

    let automaticEntryAction =
      Json.Decode.(
        string
        |> map (function
             | "Add" -> Add
             | "Remove" -> Remove
             | str ->
                 raise (DecodeError ("Unknown automatic entry action: " ^ str))))

    let automaticEntry json =
      Json_Decode_Strict.
        {
          action = json |> field "action" automaticEntryAction;
          noCost =
            json |> optionalField "noCost" bool |> Ley_Option.fromOption false;
          target = json |> field "target" automaticEntryTarget;
        }
    ...
    type multilingual = {
      ...
      permanentDemonicConsumption : permanentDemonicConsumption option;
      automaticEntries : automaticEntry list option;
      prerequisites :
      ...
      Json_Decode_Strict.
        {
          ...
          permanentDemonicConsumption =
            json
            |> optionalField "permanentDemonicConsumption"
                 permanentDemonicConsumption;
          automaticEntries =
            json |> optionalField "automaticEntries" (list automaticEntry);
          prerequisites =
            json
            |> field "prerequisites"
        ...
        ( multilingual.id,
          {
            ...
            permanentDemonicConsumption =
              multilingual.permanentDemonicConsumption;
            automaticEntries =
              multilingual.automaticEntries |> Ley_Option.fromOption [];
            prerequisites =
              Prerequisite.Collection.AdvantageDisadvantage.Decode
