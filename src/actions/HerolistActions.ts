...
export function _createHero(name: string, sex: 'm' | 'f', el: string, enableAllRuleBooks: boolean, enabledRuleBooks: Set<string>): AsyncAction {
  return (dispatch, getState) => {
    ...
    if (typeof currentId !== 'string' || typeof currentId === 'string' && typeof currentId === 'string' && past.length === 0) {
      const selectedExperienceLevel = getWikiExperienceLevels(state).get(el);
      const totalAp = selectedExperienceLevel!.ap;
