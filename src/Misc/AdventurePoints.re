...
let getDisAdvantagesSubtypeMax = (staticData, hero: Hero.t, isMagical) =>
  isMagical
    ? hero.specialAbilities
      ...
      |> listToOption
      |> option(
           defaultDisAdvantagesSubtypeMax, ((_, _, trad: MagicalTradition.t)) =>
...
let getMissingApForDisAdvantage =
  ...
  let absoluteApCost = Ley_Int.abs(apCost);
...
module Sum = {
  ...
  let getApSpentOnAttributes =
    Ley_IntMap.foldr(
      (x: Attribute.Dynamic.t) =>
        IC.getApForRange(E, ~fromValue=8, ~toValue=x.value) |> (+),
      0,
    );
    ...
    Ley_IntMap.foldr(
      (x: Skill.Dynamic.t) =>
        ...
        |> option(id, (staticEntry: Skill.Static.t) =>
             IC.getApForRange(staticEntry.ic, ~fromValue=0, ~toValue=x.value)
             |> (+)
           ),
      0,
    );
    ...
    Ley_IntMap.foldr(
      (x: Skill.Dynamic.t) =>
        ...
        |> option(id, (staticEntry: CombatTechnique.Static.t) =>
             IC.getApForRange(staticEntry.ic, ~fromValue=6, ~toValue=x.value)
             |> (+)
           ),
      0,
    );
      ...
      (x: ActivatableSkill.Dynamic.t) =>
        ...
        | Active(value) =>
          ...
          |> option(id, (staticEntry: Spell.Static.t) =>
               IC.getApForRange(staticEntry.ic, ~fromValue=0, ~toValue=value)
               |> (+)
             )
        | Inactive => id
        },
